import { Car } from './types';

export const mockCars: Car[] = [
  {
    id: 'car-123',
    make: 'Honda',
    model: 'Civic',
    year: 2019,
    trim: 'EX',
    price: 13500,
    mileage: 42000,
    location: 'Chicago, IL',
    sellerType: 'private',
    transmission: 'automatic',
    imageUrl: 'https://images.unsplash.com/photo-1590362891991-f776e747a588?auto=format&fit=crop&q=80&w=800',
    score: 87,
    scoreBreakdown: { budgetFit: 90, mileageScore: 85, reliability: 95, priceVsMarket: 80 },
    marketAvgPrice: 15000,
    recallCount: 0
  },
  {
    id: 'car-456',
    make: 'Toyota',
    model: 'Camry',
    year: 2020,
    trim: 'SE',
    price: 18000,
    mileage: 35000,
    location: 'Naperville, IL',
    sellerType: 'dealer',
    transmission: 'automatic',
    imageUrl: 'https://images.unsplash.com/photo-1621007947382-bb3c3994e3fd?auto=format&fit=crop&q=80&w=800',
    score: 75,
    scoreBreakdown: { budgetFit: 70, mileageScore: 90, reliability: 95, priceVsMarket: 60 },
    marketAvgPrice: 18500,
    recallCount: 1
  },
  {
    id: 'car-789',
    make: 'Mazda',
    model: 'CX-5',
    year: 2018,
    trim: 'Touring',
    price: 16500,
    mileage: 55000,
    location: 'Evanston, IL',
    sellerType: 'private',
    transmission: 'automatic',
    imageUrl: 'https://images.unsplash.com/photo-1542282088-fe8426682b8f?auto=format&fit=crop&q=80&w=800',
    score: 55,
    scoreBreakdown: { budgetFit: 50, mileageScore: 60, reliability: 80, priceVsMarket: 50 },
    marketAvgPrice: 16000,
    recallCount: 0
  },
  {
    id: 'car-101',
    make: 'Ford',
    model: 'Focus',
    year: 2016,
    trim: 'SE',
    price: 8500,
    mileage: 82000,
    location: 'Aurora, IL',
    sellerType: 'dealer',
    transmission: 'automatic',
    imageUrl: 'https://images.unsplash.com/photo-1552519507-da3b142c6e3d?auto=format&fit=crop&q=80&w=800',
    score: 35,
    scoreBreakdown: { budgetFit: 90, mileageScore: 30, reliability: 30, priceVsMarket: 80 },
    marketAvgPrice: 9000,
    recallCount: 3
  },
  {
    id: 'car-202',
    make: 'Subaru',
    model: 'Outback',
    year: 2021,
    trim: 'Premium',
    price: 24000,
    mileage: 28000,
    location: 'Schaumburg, IL',
    sellerType: 'dealer',
    transmission: 'automatic',
    imageUrl: 'https://images.unsplash.com/photo-1533473359331-0135ef1b58bf?auto=format&fit=crop&q=80&w=800',
    score: 82,
    scoreBreakdown: { budgetFit: 60, mileageScore: 95, reliability: 90, priceVsMarket: 85 },
    marketAvgPrice: 25500,
    recallCount: 0
  }
];
